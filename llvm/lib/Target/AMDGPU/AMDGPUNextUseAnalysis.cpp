...
static std::string nameForMBB(const llvm::MachineBasicBlock *BB,
                              ModuleSlotTracker &MST) {
  return BB ? nameForMBB(*BB, MST) : std::string("<nullptr>");
}

static inline std::string nameForLoop(MachineLoop *L, ModuleSlotTracker &MST) {
  if (!L)
    return std::string("<no-loop>");
  return "loop-(" + nameForMBB(L->getHeader(), MST) + ")";
}
...
struct LiveRegUse {
  ...
  bool operator<(const LiveRegUse &Other) const {
    ...
    // Ugh. In mlMode PHIs and the first non-PHI instruction have id 0. In this
    // case, consider PHIs as less than the first non-PHI instruction.
...
private:
  ...
  std::string toString(EdgeKind EK) {
    switch (EK) {
    case Back:
      return "Back";
    case None:
  ...
  /// Helper: Calculate exit from loop considering simple vs nested loop
  /// structure
  MBBDistPair calcLoopExitForOutsideUse(const MachineBasicBlock *CurMBB,
                                        const MachineBasicBlock *UseMBB,
                                        MachineLoop *CurLoop) const {
    if (CurLoop->getSubLoops().empty() && CurLoop->isOutermost())
      return calcLoopWeightedDistanceAndExitingLatch(CurMBB);
    return calcNestedLoopDistanceAndExitingLatchToOutsideUse(CurMBB, UseMBB);
  }

  /// Helper: Calculate complete distance from loop exit to use instruction
  double calcLoopExitDistanceToUse(const MachineBasicBlock *CurMBB,
                                   const MachineBasicBlock *UseMBB,
                                   const MachineInstr *UseMI,
                                   MachineLoop *CurLoop) const {
    MBBDistPair Exit = calcLoopExitForOutsideUse(CurMBB, UseMBB, CurLoop);
    return calcDistanceFromExitToUse(Exit, UseMBB, UseMI);
  }

  /// Helper: Calculate total distance from exit point to use instruction
  double calcDistanceFromExitToUse(const MBBDistPair &Exit,
                                   const MachineBasicBlock *UseMBB,
                                   const MachineInstr *UseMI) const {
  ...
  double calcNestedLoopDistanceToOutsideUse(const MachineBasicBlock *CurMBB,
                                            const MachineBasicBlock *UseMBB,
                                            const MachineInstr *UseMI) const {
    MBBDistPair Exit =
        calcNestedLoopDistanceAndExitingLatchToOutsideUse(CurMBB, UseMBB);
    return calcDistanceFromExitToUse(Exit, UseMBB, UseMI);
  }
  ...
  double calcNestedLoopDistanceToParentUse(const MachineBasicBlock *CurMBB,
                                           const MachineBasicBlock *UseMBB,
                                           const MachineInstr *UseMI) const {
    MBBDistPair Exit =
  ...
  double calcInsideToOutsideLoopDistance(
      Register LiveReg, LaneBitmask LiveLaneMask, const MachineInstr *DefMI,
      const MachineInstr *UseMI, const MachineBasicBlock *PhiUseEdge) const;
  ...
  //----------------------------------------------------------------------------
  // getUses helpers (ML mode)
  //----------------------------------------------------------------------------
  bool isUseMLModeReachable(const MachineInstr &MI,
                            const MachineBasicBlock *MBB,
...
void AMDGPUNextUseAnalysisImpl::initializePathInfo(PathInfo &Slot, Path P,
                                                   EdgeKind EK) const {
  Slot.EK = EK;
...
void AMDGPUNextUseAnalysisImpl::initializePathsFromMF() {
  Paths.clear();
...
bool AMDGPUNextUseAnalysisImpl::calcIsReachable(const MachineBasicBlock *From,
                                                const MachineBasicBlock *To,
...
double AMDGPUNextUseAnalysisImpl::calcInsideToOutsideLoopDistance(
  ...
  if (isUseOutsideOfTheCurrentLoopNest(UseLoop, CurLoop))
    return calcLoopExitDistanceToUse(CurMBB, UseMBB, UseMI, CurLoop);
  ...
  if (isUseInParentLoop(UseLoop, CurLoop)) {
    ...
    return calcNestedLoopDistanceToParentUse(CurMBB, UseMBB, UseMI);
  }
...
static std::string printRegToString(Register Reg, unsigned SubRegIdx,
...
const MachineBasicBlock *mbbForPhiOp(const MachineOperand *MO) {
...
double AMDGPUNextUseAnalysisImpl::calcDistanceToUse(
  ...
  if (mlMode()) {
    ...
    //--------------------------------------------------------------------------
    if (!CurLoop && UseLoop) {
      MachineLoop *TopLoop = UseLoop->getOutermostLoop();
    ...
    if (!CurLoop->contains(UseLoop)) {
      if (CurLoop->getSubLoops().empty() && CurLoop->isOutermost()) {
        MBBDistPair Exit = calcLoopWeightedDistanceAndExitingLatch(CurMBB);
        MachineLoop *TopLoop = UseLoop->getOutermostLoop();
        const MachineBasicBlock *PreHdr = TopLoop->getLoopPreheader();
        return calcDistanceFromExitToUse(Exit, PreHdr, &PreHdr->back());
      }
      return calcNestedLoopDistanceToOutsideUse(CurMBB, UseMBB, UseMI);
    }
...
bool AMDGPUNextUseAnalysisImpl::isUseMLModeReachable(
    const MachineInstr &MI, const MachineBasicBlock *MBB,
    const MachineOperand *UseMO, const MachineInstr *UseMI,
    const MachineBasicBlock *UseMBB) const {
  ...
  // PHI uses are considered part of the incoming BB. Check for reachability at
  // the edge.
  if (UseMI->isPHI()) {
  ...
  if (MBB == UseMBB) {
    ...
    // A Def in the loop means that the value at MI will not survive through to
    // this use.
    MachineLoop *UseLoop = MLI->getLoopFor(UseMBB);
...
void AMDGPUNextUseAnalysisImpl::getUses(
  ...
  for (const MachineOperand *UseMO : getRegisterUses(Reg)) {
    ...
    bool Reachable;
    if (mlMode())
      Reachable = isUseMLModeReachable(MI, MBB, UseMO, UseMI, UseMBB);
    else if (MBB == UseMBB)
